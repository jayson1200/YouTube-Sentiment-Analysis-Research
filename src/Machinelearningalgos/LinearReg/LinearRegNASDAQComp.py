...
y = np.array(trainingData.drop(["Tone", "DateTime","DJIAChange","S&P500Change"], axis = 1).values.tolist())

regression = LinearRegression().fit(X,y)

X_test = np.array(testingData.drop(["DJIAChange", "DateTime","NASDAQChange","S&P500Change"], axis = 1).values.tolist())
y_test = np.array(testingData.drop(["Tone", "DateTime","DJIAChange","S&P500Change"], axis = 1).values.tolist())
...
plt.title('Linear Regression NASDAQ Comp Change Prediction')
plt.xlabel('Time')
plt.ylabel('NASDAQ Comp Change')
plt.legend()
plt.show()